...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbankrobbery = "https://discordapp.com/api/webhooks/768815416020697099/AOvlzDGHS4xSezst7MhkW5biUhhJWXckaCE7HQV-yAz91_5zCl1xxIqUl7jP1U7efWph"
        ...
        for l,w in pairs(soldado) do
            ...
            if player then
                async(function()
                    ...
                    TriggerClientEvent('chatMessage',player,"Dispatch",{65,130,255},"Roubo ao Banco Central iniciado, dirija-se até o local e intercepte os assaltantes.")
...
function vRPrb.updateCops(message)
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        TriggerClientEvent('chatMessage', -1, "[APNEWS BANCO CENTRAL]", {64,64,255}, message)
    end
end
...
function generateRandomMoney(src, bank)
    ...
    while true do
        local randommoney = math.random(150000, 200000)
