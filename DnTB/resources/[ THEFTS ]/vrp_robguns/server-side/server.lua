...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookrouboarmas = "https://discordapp.com/api/webhooks/745624892904308808/c7ET8MpIsWtjcBzCkgyootaIMTyUb3aTuf6tEn8F_9tgISsD9GEi5dgzFtb-m1Gss10U"
...
function src.checkPolice()
	...
	if user_id then
		...
		if #policia <= 1 then
			...
			return false
		elseif (parseInt(os.time())-timedown) <= 1000 then
			TriggerClientEvent("Notify",source,"importante","Aguarde "..vRP.getMinSecs(parseInt(1000-(os.time()-timedown)))..".",8000)
			return false
		end
	end
	return true
			...
			if robbery then
				...
				if parseInt(math.random(100)) <= 10 then
					...
					if parseInt(weaponsort) == 1 then
						...
						vRP.giveInventoryItem( user_id,"wammo|WEAPON_SNSPISTOL",20)
