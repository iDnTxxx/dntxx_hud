...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbankrobbery = "https://discordapp.com/api/webhooks/768799211172003840/D5FzpLKLmiZO00EylGMTkXn2vI4Srg0OS7mCJG_4ouO88NGSnpw1PPHq25VCVLlBpPH9"
...
function func.CheckPayment(bankId)
    ...
    if user_id then
        if parseInt(moneyHeist[bankId]) <= parseInt(BankHeists[bankId]["Money"]) then
            local cashRecieved = math.random(228000,556000)
        ...
        for l,w in pairs(policia) do
            ...
            if player then
                async(function()
                    ...
                    TriggerClientEvent('chatMessage',player,"Dispatch",{65,130,255},"O roubo começou no ^1"..bankId.."^0, dirija-se até o local e intercepte os assaltantes.")
                    --TriggerClientEvent("NotifyPush",player,{ code = 31, title = "O roubo começou no "..bankId..", dirija-se até o local e intercepte os assaltantes.", x = x, y = y, z = z, badge = "Banco "..bankId.." " })
...
function func.KeyCard()
	...
	if user_id then
        if vRP.getInventoryItemAmount(user_id,"keycard") >= 1 then
            ...
            SendWebhookMessage(webhookbankrobbery,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[BANCO]: "..bankId.." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
			return true 
		else
			TriggerClientEvent("Notify",source,"negado","Você não possui o item <b>Key-Card</b>.") 
			return false
		end
	end
end
