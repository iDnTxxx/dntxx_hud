	...
	if user_id then
		...
		if #policia >= 12 then
			if timers[id] == 0 or not timers[id] then
				timers[id] = 5000
				TriggerClientEvent('iniciandoniobio',source,x,y,z)
					...
					for l,w in pairs(policia) do
						...
						if player then
							async(function()
								...
								TriggerClientEvent('chatMessage',player,"911",{64,64,255},"O roubo começou na ^1Niobio^0, dirija-se até o local e intercepte o assaltante.")
				...
				SetTimeout(160000,function()
					if user_id then
						local identity = vRP.getUserIdentity(user_id)
						local randmoney = math.random(1000000,1000000)
